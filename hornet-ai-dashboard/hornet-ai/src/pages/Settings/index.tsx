          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-xl">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-green-700 dark:text-green-400 font-medium">Connection Status</span>
              </div>
              <p className="text-xs text-gray-500 mt-1">Click "Test Connection" to verify your Flask API</p>
              <button className="mt-2 px-3 py-1 text-xs bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">
                Test Connection
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <div>
              <label className="text-sm text-gray-600 dark:text-gray-400 mb-1 block">
                Alert Threshold (confidence %) — current: {settings.alertThreshold}%
              </label>
              <input
                type="range"
              ...
              <div key={opt.key} className="flex items-center justify-between py-2">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    settings[opt.key as keyof typeof settings] ? "bg-brand-500" : "bg-gray-200 dark:bg-gray-700"
                  }`}
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    settings[opt.key as keyof typeof settings] ? "translate-x-6" : "translate-x-1"
                  }`} />
                </button>
              </div>
            ))}
        ...
        <div className="col-span-12 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {settings.cameras.map((cam) => (
              <div key={cam.id} className={`p-4 rounded-xl border ${cam.enabled ? "border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/10" : "border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50"}`}>
                <div className="flex items-center justify-between mb-2">
                  <span className="font-medium text-sm text-gray-800 dark:text-white">{cam.name}</span>
                  <button
                    onClick={() => setSettings(prev => ({
                      ...prev,
                      cameras: prev.cameras.map(c => c.id === cam.id ? { ...c, enabled: !c.enabled } : c)
                    }))}
                    className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${cam.enabled ? "bg-green-500" : "bg-gray-300 dark:bg-gray-600"}`}
                  >
