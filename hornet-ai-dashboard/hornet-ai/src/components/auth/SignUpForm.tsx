...
import { Link, useNavigate } from "react-router"; // 1. Bổ sung useNavigate ở đây
import { Shield, ArrowLeft, Eye, EyeOff, Mail, Lock, User } from "lucide-react";

export default function SignUpForm() {
  const [showPassword, setShowPassword] = useState(false);
  const [isChecked, setIsChecked] = useState(false);
  
  // 2. Khởi tạo công cụ chuyển trang
  const navigate = useNavigate(); 

 // Thêm type React.FormEvent<HTMLFormElement> vào e
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col items-center justify-center p-4 font-sans text-gray-900 py-10">
      ...
      <div className="w-full max-w-md bg-white border border-gray-200 rounded-2xl p-6 sm:p-8 shadow-xl">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Join Hornet3000 System</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          <button 
            type="button" // Thêm type="button" để không bị nhầm là nút Submit form
            onClick={handleSocialLogin} // Gắn sự kiện click
          ...
          <button 
            ...
            className="flex items-center justify-center gap-3 py-2.5 text-sm font-semibold text-gray-700 transition-colors bg-white border border-gray-300 shadow-sm rounded-lg hover:bg-gray-50"
          >
            <svg width="18" height="18" viewBox="0 0 21 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="relative flex items-center py-2 mb-6">
          ...
          <span className="flex-shrink-0 px-4 text-xs text-gray-500 uppercase tracking-wider bg-white">Or register with email</span>
          <div className="flex-grow border-t border-gray-200"></div>
        </div>

        {/* 5. Gắn sự kiện onSubmit vào thẻ form */}
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="flex items-start gap-3 pt-2">
            <input
              type="checkbox"
              required // Bắt buộc người dùng phải tick đồng ý điều khoản
              checked={isChecked}
            ...
            <p className="text-sm font-medium text-gray-600">
              ...
              <Link to="#" className="text-blue-600 hover:text-blue-700 transition-colors">Terms and Conditions</Link>
              , and our{" "}
              <Link to="#" className="text-blue-600 hover:text-blue-700 transition-colors">Privacy Policy</Link>.
            </p>
          </div>

          <button type="submit" className="w-full flex items-center justify-center py-2.5 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold transition-colors mt-2 shadow-md shadow-blue-500/20">
            Create Account
          </button>
        </form>
        ...
        <div className="mt-8 text-center">
          <p className="text-sm font-medium text-gray-500">
            ...
            <Link to="/signin" className="font-bold text-blue-600 hover:text-blue-700 transition-colors">Sign In</Link>
          </p>
        </div>
