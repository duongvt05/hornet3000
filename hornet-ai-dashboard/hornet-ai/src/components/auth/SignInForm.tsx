...
export default function SignInForm() {
  ...
  // Hàm xử lý submit form có khai báo type chuẩn của TypeScript
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault(); // Ngăn trình duyệt reload trang
    
    // Giả lập xử lý đăng nhập thành công và chuyển về trang chủ (Dashboard)
    navigate("/");
  };

  // Hàm xử lý khi bấm đăng nhập bằng MXH
      ...
      <div className="w-full max-w-md bg-white border border-gray-200 rounded-2xl p-6 sm:p-8 shadow-xl">
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            <Link to="/reset-password" className="text-sm font-semibold text-blue-600 hover:text-blue-700 transition-colors">
              Forgot password?
            </Link>
          </div>
          ...
          <button 
            type="submit" 
            className="w-full flex items-center justify-center py-2.5 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold transition-colors mt-2 shadow-md shadow-blue-500/20"
          >
